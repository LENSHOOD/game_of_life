...
public class Fleet implements BlockHelper {
    ...
    public Map<Block.Point, Block> generateBlocks(int size) {
        ...
        // bottom line
        deploy(SpaceShips.MiddleWeight, 20, 0, battleFlied);
        deploy(SpaceShips.MiddleWeight, 30, 0, battleFlied);
        deploy(SpaceShips.HeavyWeight, 40, 0, battleFlied);
        deploy(SpaceShips.HeavyWeight, 50, 0, battleFlied);
        deploy(SpaceShips.HeavyWeight, 60, 0, battleFlied);
        deploy(SpaceShips.MiddleWeight, 70, 0, battleFlied);
        deploy(SpaceShips.MiddleWeight, 80, 0, battleFlied);

        // second line
        deploy(SpaceShips.MiddleWeight, 30, 10, battleFlied);
        deploy(SpaceShips.MiddleWeight, 40, 10, battleFlied);
        deploy(SpaceShips.MiddleWeight, 50, 10, battleFlied);
        deploy(SpaceShips.MiddleWeight, 60, 10, battleFlied);
        deploy(SpaceShips.MiddleWeight, 70, 10, battleFlied);

        // first line
        deploy(SpaceShips.LightWeight, 40, 20, battleFlied);
        deploy(SpaceShips.LightWeight, 50, 20, battleFlied);
        deploy(SpaceShips.LightWeight, 60, 20, battleFlied);

        // corner
        deploy(SpaceShips.Glider, 1, 0, battleFlied);
        deploy(SpaceShips.Glider, 90, 0, battleFlied);

        return battleFlied;
    }

    private void deploy(SpaceShips type, int locationX, int locationY, Map<Block.Point, Block> battleFlied) {
        dock.get(type).generate(locationX, locationY, battleFlied);
    }
}
